...
queryData <- data.frame("doc_id" = "query", "text" = query)
vquery <- DataframeSource(queryData)
...
#df[[N.docs]]$content <- qdf[[1]]$content

aCorpus <- as.VCorpus(allDf)

## create term-document matrix
tdm <- TermDocumentMatrix(df)
tmp <- removeSparseTerms(tdm, 0.94)
inspect(tdm[0:4,])
inspect(tmp[0:4,])
...
most_frecuent_matrix_top50<- head(most_frecuent_matrix[,c(42300,42308)], n = 10)
